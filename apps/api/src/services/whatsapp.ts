...
async function getOrCreateCustomer(businessId: string, phone: string, name?: string) {
  const { data, error } = await supabase.from("customers").upsert({ business_id: businessId, phone, ...(name ? { name } : {}) }, { onConflict: "business_id,phone" }).select("id,name,phone").single();
...
async function getOrCreateConversation(businessId: string, customerId: string) {
  const { data: existing, error: existingError } = await supabase.from("conversations").select("id,status").eq("business_id", businessId).eq("customer_id", customerId).order("updated_at", { ascending: false }).limit(1).maybeSingle();
  if (existingError) throw new Error(`Failed to find WhatsApp conversation: ${existingError.message}`);
  if (existing) return existing;
  const { data: conversation, error } = await supabase.from("conversations").insert({ business_id: businessId, customer_id: customerId, status: "open" }).select("id,status").single();
...
export async function processWhatsAppStatusUpdate(businessId: string, status: WhatsAppStatusUpdate) {
  const update: Record<string, string | null> = { delivery_status: status.status };
  ...
  if (status.status === "failed") { update.failed_at = timestamp; update.delivery_error = status.error ?? "WhatsApp delivery failed."; }
...
export async function processIncomingWhatsAppMessage(businessId: string, incoming: WhatsAppTextMessage) {
  const { data: duplicate, error: duplicateError } = await supabase.from("messages").select("id,conversation_id").eq("external_message_id", incoming.messageId).maybeSingle();
  if (duplicateError) throw new Error(`Failed to check WhatsApp message duplicate: ${duplicateError.message}`);
  if (duplicate) return { duplicate: true, customer: null, conversation: { id: duplicate.conversation_id, status: "unknown" }, message: duplicate, automation: { matched: false, skipped: true, reason: "duplicate" } };
  const customer = await getOrCreateCustomer(businessId, incoming.from, incoming.name);
  const conversation = await getOrCreateConversation(businessId, customer.id);
  ...
  if (match.actionType === "send_reply" && match.responseText?.trim()) delivery = await sendWhatsAppText(incoming.from, match.responseText.trim());
  const { error: logError } = await supabase.from("automation_logs").insert({ business_id: businessId, rule_id: match.rule?.id ?? null, conversation_id: conversation.id, message_id: message.id, input_text: incoming.text, matched: true, response_text: match.responseText, action_type: match.actionType, action_result: { ...actionResult.result, whatsappDelivery: delivery ? { mode: delivery.mode, sent: delivery.sent, messageId: delivery.messageId } : null } });
...
export function parseWhatsAppWebhook(body: any): { messages: WhatsAppTextMessage[]; statuses: WhatsAppStatusUpdate[] } {
  ...
  for (const entry of Array.isArray(body?.entry) ? body.entry : []) for (const change of Array.isArray(entry?.changes) ? entry.changes : []) {
    const value = change?.value, contacts = Array.isArray(value?.contacts) ? value.contacts : [], contactNames = new Map<string, string>();
    for (const contact of contacts) { const waId = typeof contact?.wa_id === "string" ? contact.wa_id : ""; const name = typeof contact?.profile?.name === "string" ? contact.profile.name : ""; if (waId && name) contactNames.set(waId, name); }
    const phoneNumberId = typeof value?.metadata?.phone_number_id === "string" ? value.metadata.phone_number_id : undefined;
    for (const message of Array.isArray(value?.messages) ? value.messages : []) { if (message?.type !== "text" || typeof message?.from !== "string" || typeof message?.id !== "string" || typeof message?.text?.body !== "string") continue; messages.push({ messageId: message.id, from: message.from, name: contactNames.get(message.from), text: message.text.body, phoneNumberId, timestamp: typeof message.timestamp === "string" ? message.timestamp : undefined }); }
    for (const status of Array.isArray(value?.statuses) ? value.statuses : []) { if (typeof status?.id !== "string" || !["sent","delivered","read","failed"].includes(status?.status)) continue; statuses.push({ messageId: status.id, status: status.status, timestamp: typeof status.timestamp === "string" ? status.timestamp : undefined, error: Array.isArray(status?.errors) ? status.errors[0]?.title || status.errors[0]?.message : undefined, phoneNumberId }); }
  }
