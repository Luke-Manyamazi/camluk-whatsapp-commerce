...
export async function getConversations(
  ...
  if (term) {
    ...
    if (customerIds.length === 0) {
      return { conversations: [], pagination: { page: safePage, limit: safeLimit, total: 0, totalPages: 0 } };
    }
  }

  let query = supabase
    .from("conversations")
    .select(`id, status, created_at, updated_at, customers (name, phone), messages (content, created_at)`, { count: "exact" })
    .eq("business_id", businessId);
  ...
  const conversations = (data ?? []).map((conversation) => {
    ...
    const latestMessage = messages.reduce<typeof messages[number] | undefined>((latest, message) => {
      if (!latest || new Date(message.created_at).getTime() > new Date(latest.created_at).getTime()) return message;
      return latest;
    }, undefined);
