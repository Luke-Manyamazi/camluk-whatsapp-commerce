  ...
  try {
    ...
    if (membershipError) {
      ...
      res.status(500).json({
        ...
        message: "Unable to determine business membership."
      });
      return;
    }
    ...
    if (!membership) {
      res.status(403).json({
        error: "Forbidden",
        message: "The authenticated user does not belong to a business."
      });
      return;
    }
    ...
    if (!["owner", "admin", "member"].includes(role)) {
      res.status(403).json({
        error: "Forbidden",
        message: "The user's business role is invalid."
      });
      return;
    }
