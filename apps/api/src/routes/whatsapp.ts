...
import { verifyWhatsAppWebhookToken } from "../services/whatsapp-channels.js";
...
router.get("/webhook", async (req: Request, res: Response) => {
  ...
  try {
    const channel = await verifyWhatsAppWebhookToken(token);
    if (channel) return res.status(200).send(challenge);
    const legacyToken = process.env.WHATSAPP_VERIFY_TOKEN?.trim();
    if (legacyToken && token === legacyToken) return res.status(200).send(challenge);
    return res.status(403).send("Forbidden");
  } catch (error) { console.error("Failed to verify WhatsApp webhook:", error); return res.status(500).json({ message: "Webhook verification failed." }); }
});
...
router.post("/webhook", async (req: Request, res: Response) => {
  ...
  try {
    ...
    for (const status of parsed.statuses) {
      const channel = await resolveWhatsAppChannelByPhoneNumberId(status.phoneNumberId);
      if (!channel) continue;
      await processWhatsAppStatusUpdate(channel.businessId, status);
    }
    for (const incoming of parsed.messages) {
      const channel = await resolveWhatsAppChannelByPhoneNumberId(incoming.phoneNumberId);
      if (!channel) { console.warn("Ignoring WhatsApp message for unknown phone number", { phoneNumberId: incoming.phoneNumberId }); continue; }
      const result = await processIncomingWhatsAppMessage(channel.businessId, incoming, channel);
      console.log("Processed WhatsApp message", { messageId: incoming.messageId, conversationId: result.conversation.id, businessId: channel.businessId, channelId: channel.id, matched: result.automation.matched });
    }
